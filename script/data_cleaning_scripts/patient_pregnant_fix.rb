#update records of patients with incomplete preganancy data
...
EOF
	...
	(pregnant_obs || []).each do |patient|
		puts "Working on patient_id: #{patient["person_id"].to_i}"

		update_encounter(patient['person_id'].to_i, patient['encounter_id'].to_i, patient['value_coded'], patient['obs_id'].to_i)
		
		puts "Finished working on patient_id: #{patient["person_id"].to_i}"
	end
end
...
EOF
		...
		elsif value.to_i == 1066
			value_coded_name = 1103 
		elsif value.to_i == 1067
